...
namespace Engine
{
	const Engine::TickGroups& TickGroups::get()
	{
#ifdef DEBUG_BUILD
		...
		assert(EngineBase::get().getTickGroupManager().areTickGroupsInitialized());
#endif
		...
		return EngineBase::get().tickGroups();
	}
	...
	{
		if (EngineBase::get().getTickGroupManager().registerTickGroup(*this, TickGroupManager::TickGroupKey{}))
		{
			bRegistered = true;
		}
		else
