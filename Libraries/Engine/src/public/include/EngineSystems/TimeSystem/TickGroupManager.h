...
namespace Engine
{
	...
	{
	public:
		/** Key allowing only the EngineBase to call certain functions */
	struct EngineBaseKey { private: EngineBaseKey() {} friend class EngineBase; };
	struct TickGroupKey { private: TickGroupKey() {} friend TickGroupDefinition; };
	public:
		void start_TickGroupRegistration(const EngineBaseKey& privateKey);
		void stop_TickGroupRegistration(const EngineBaseKey& privateKey);
